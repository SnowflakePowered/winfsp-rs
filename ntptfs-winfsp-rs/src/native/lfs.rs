...
use windows::Win32::Storage::FileSystem::{
    FILE_ACCESS_RIGHTS, FILE_ATTRIBUTE_REPARSE_POINT, 
    FILE_END_OF_FILE_INFO, FILE_READ_ATTRIBUTES, FILE_RENAME_INFO, FILE_SHARE_DELETE,
...
pub fn lfs_open_file<P: AsRef<U16CStr>>(
    ...
    unsafe {
        NtOpenFile(
            handle.deref_mut(),
            (FILE_READ_ATTRIBUTES | desired_access).0,
            &mut object_attrs,
            iosb.as_mut_ptr(),
...
pub fn lfs_get_file_info<'a, P: Into<MaybeOpenFileInfo<'a>>>(
    ...
    if let Err(e) = result.as_ref() && e.code().to_ntstatus() != STATUS_BUFFER_OVERFLOW {
        return Err(FspError::NTSTATUS(e.code().to_ntstatus()))
    }
